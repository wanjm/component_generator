...
''';
  ...
  String _generateMethod(MethodElement method) {
    ...
    if (metadataDynamic is Iterable) {
      for (final annotation in metadataDynamic) {
        final elementAnnotation = annotation as ElementAnnotation;
        final constantValue = elementAnnotation.computeConstantValue();
        if (constantValue != null) {
          final annotationType = constantValue.type;
          if (annotationType != null && annotationType.element?.name == 'ReqConfig') {
            reqConfigAnnotation = elementAnnotation;
            break;
          }
        }
      }
    ...
    if (reqConfigAnnotation == null) {
      throw InvalidGenerationSourceError(
        '方法 ${method.name} 必须添加 @ReqConfig 注解',
        element: method,
      );
    }
